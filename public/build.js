...
var canvasFactory=require('./canvasFactory');var emotions=require('../listemotions.json');var meinVue=new Vue({el:'#channelOverview',data:{activeChannels:[],emotions:emotions,emotionChannels:{}},ready:function ready(){this.fetchChannels()},methods:{fetchChannels:function fetchChannels(){this.$http.get('/overview/activeChannels/').success(function(channels){this.$set("activeChannels",channels);this.$nextTick(this.drawThumbnailsActive)}).error(function(error){this.$set("activeChannels",require("./userMock.json").activeChannels);this.$nextTick(this.drawThumbnailsActive)});this.$http.get('/overview/emotionChannels/').success(function(channels){this.$set("emotionChannels",channels);this.$nextTick(this.drawThumbnailsEmotion)}).error(function(error){})},drawThumbnailsActive:function drawThumbnailsActive(){var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=this.activeChannels[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var channel=_step.value;var canvas=document.getElementById("thumbnail_active_"+channel.name);canvasFactory.createThumbnail(canvas,channel)}}catch(err) {_didIteratorError=true;_iteratorError=err}finally {try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return()}}finally {if(_didIteratorError){throw _iteratorError}}}},drawThumbnailsEmotion:function drawThumbnailsEmotion(){var _iteratorNormalCompletion2=true;var _didIteratorError2=false;var _iteratorError2=undefined;try{for(var _iterator2=this.emotions[Symbol.iterator](),_step2;!(_iteratorNormalCompletion2=(_step2=_iterator2.next()).done);_iteratorNormalCompletion2=true){var emotion=_step2.value;var channel=this.emotionChannels[emotion];if(channel){var canvas=document.getElementById("thumbnail_emotion_"+channel.name);canvasFactory.createThumbnail(canvas,channel)}}}catch(err) {_didIteratorError2=true;_iteratorError2=err}finally {try{if(!_iteratorNormalCompletion2&&_iterator2.return){_iterator2.return()}}finally {if(_didIteratorError2){throw _iteratorError2}}}}}})},{"../listemotions.json":6,"./canvasFactory":4,"./userMock.json":5,"vue":16,"vue-resource":9}],4:[function(require,module,exports){"use strict";var Chart=require('chart.js');var TransformOptions=require('./TransformOptions');exports.createThumbnail=function createThumbnail(canvas,channel){var ctx=canvas.getContext("2d"); // Old positions
var positions=[new TransformOptions(canvas.width,canvas.height,2/3,2/3),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,0),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,1/3),new TransformOptions(canvas.width,canvas.height,1/3,1/3,0,2/3),new TransformOptions(canvas.width,canvas.height,1/3,1/3,1/3,2/3),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,2/3)];positions=[new TransformOptions(canvas.width,canvas.height,2/3,1),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,0),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,1/3),new TransformOptions(canvas.width,canvas.height,1/3,1/3,2/3,2/3)];var drawFunctions=[drawChannelLogo,drawFractal,drawDiagram1,drawDiagram2 //,
//drawDiagram3,
//drawDiagram4
];for(var i=0;i<positions.length;i++){drawFunctions[i](channel,ctx,positions[i])}};function drawChannelLogo(channel,ctx,options){var img=new Image();img.addEventListener("load",function(){ctx.save();options.apply(ctx);ctx.drawImage(img,0,0,options.width,options.height);ctx.restore()},false);img.src=channel.logo}function drawFractal(channel,ctx,options){ // Dummy
ctx.save();options.apply(ctx);ctx.fillStyle="red";ctx.fillRect(0,0,options.width,options.height);ctx.fillStyle="rgb(0,0,0)";ctx.fillText(channel.name,10,10);ctx.restore()}function drawDiagram1(channel,ctx,options){ctx.save();options.apply(ctx);var data=getEmotions(channel.name); // Building a new Canvas for the Diagramm
...
var newCtx=diagramCanvas.getContext("2d"); // Build the chart
var chartOptions={scaleShowGridLines:false,scaleShowLabels:false,scaleFontSize:0,animation:false,responsive:true};var chart=new Chart(newCtx).Bar(data,chartOptions); // Set the barcolors
var dataColors=["blue","green","yellow","orange","red"]; // in getData unterbringen
for(var i=0;i<chart.datasets[0].bars.length;i++){chart.datasets[0].bars[i].fillColor=dataColors[i]}chart.update(); // Draw the diagram and hide the diagram canvas
ctx.drawImage(diagramCanvas,0,0,options.width,options.height);diagramCanvas.style.display="none";ctx.restore()}function drawDiagram2(channel,ctx,options){ctx.save();options.apply(ctx);var data=getEmotions(channel.name); // Building a new Canvas for the Diagramm
var diagramCanvas=document.createElement("canvas");diagramCanvas.height=options.height;diagramCanvas.width=options.width;document.body.appendChild(diagramCanvas);var newCtx=diagramCanvas.getContext("2d"); // Build the chart
var chartOptions={scaleShowLine:false,scaleShowLabels:false,scaleFontSize:0,animation:false,responsive:true};var chart=new Chart(newCtx).Radar(data,chartOptions); // Draw the diagram and hide the diagram canvas
ctx.drawImage(diagramCanvas,0,0,options.width,options.height);diagramCanvas.style.display="none";ctx.restore()}function drawDiagram3(channel,ctx,options){ // Dummy
ctx.save();options.apply(ctx);ctx.fillStyle="green";ctx.fillRect(0,0,options.width,options.height);ctx.fillStyle="rgb(0,0,0)";ctx.fillText(channel.name,10,10);ctx.restore()}function drawDiagram4(channel,ctx,options){ // Dummy
ctx.save();options.apply(ctx);ctx.fillStyle="grey";ctx.fillRect(0,0,options.width,options.height);ctx.fillStyle="rgb(0,0,0)";ctx.fillText(channel.name,10,10);ctx.restore()}function getEmotions(channelName){return {labels:["emotion1","emotion2","emotion3","emotion4","emotion5"],datasets:[{fillColor:"rgba(220,220,220,0.5)",strokeColor:"rgba(220,220,220,0.8)",highlightFill:"rgba(220,220,220,0.75)",highlightStroke:"rgba(220,220,220,1)",data:[65,10,80,31,56],dataColors:["blue","green","yellow","orange","red"]}]}} /* saved for later:
 ...
 */},{"./TransformOptions":2,"chart.js":7}],5:[function(require,module,exports){module.exports={"activeChannels":[{"name":'gronkhtv',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/gronkhtv-profile_image-9883ecc1f767f4ff-300x300.png',"viewercount":1000,"streamid":'19083275'},{"name":'peterle',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/peterle-profile_image-61cad7802f03f0fc-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'ichzockehiernur',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/ichzockehiernur-profile_image-ac4297b139631941-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'cartmanzbs',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/cartmanzbs-profile_image-75eb906ef03865c6-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'nomadtv',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/nomadtv-profile_image-1d66198249ba36a6-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'starladder_hs-ru',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/starladder_hs_ru-profile_image-0d713d4a3d9189c5-300x300.png',"viewercount":1000,"streamid":'1290384756'}],"emotionChannels":[{"name":'gronkhtv',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/gronkhtv-profile_image-9883ecc1f767f4ff-300x300.png',"viewercount":1000,"streamid":'19083275'},{"name":'peterle',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/peterle-profile_image-61cad7802f03f0fc-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'ichzockehiernur',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/ichzockehiernur-profile_image-ac4297b139631941-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'cartmanzbs',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/cartmanzbs-profile_image-75eb906ef03865c6-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'nomadtv',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/nomadtv-profile_image-1d66198249ba36a6-300x300.jpeg',"viewercount":1000,"streamid":'1290384756'},{"name":'starladder_hs-ru',"logo":'http://static-cdn.jtvnw.net/jtv_user_pictures/starladder_hs_ru-profile_image-0d713d4a3d9189c5-300x300.png',"viewercount":1000,"streamid":'1290384756'}]}},{}],6:[function(require,module,exports){module.exports=["amused","annoyed","bored","embarassed","excited","happy","loving","provoking","rage","sad","surprised"]},{}],7:[function(require,module,exports){ /*!
 * Chart.js
 * http://chartjs.org/
 * Version: 1.0.2
 *
 ...
 * @param {Element} el
 * @param {String} cls
 */function setClass(el,cls){ /* istanbul ignore if */if(isIE9&&el.hasOwnProperty('className')){el.className=cls}else {el.setAttribute('class',cls)}} /**
...
this.el.className=removeTags(this.el.className).trim().replace(/\s+/g,' ')}} // copy def properties
...
partial:partial};Vue.version='1.0.11'; /**
